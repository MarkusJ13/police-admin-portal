...
export class Station extends React.Component{
	...
	addNewChild = e => {
		...
		firebaseApp.database().ref().child(path).push().set({name, child: []}).then(snapshot=>{
			console.log("Added")
		}).catch(err => {
	...
	addChildOfChild = (key, name) => {
		...
		firebaseApp.database().ref().child(path).push().set({name}).then(snapshot=>{
			this.toggleAddChild()
		}).catch(err => {
			console.log("some error", err)
		})
	...
	render(){
		...
		const {stations, isExpanded, isEditing, isAddingChild, newChildName} = this.state
		if (isEditing) {
			return <AddStation
				mode="edit"
				child={child}
		...
		return <ListItem isExpanded={isExpanded} style={{ height: isExpanded ? "250px" : undefined }}>
